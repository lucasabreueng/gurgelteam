"use client";

import { HiMagnifyingGlass } from "react-icons/hi2";
import { TeamServiceMock } from "@/services/team/teamService";
import { useStaffRoleOptions } from "@/lib/query/hooks/use-staff-role-options";
import { FilterBox, filterFieldHeightClass, filtersActive } from "@/components/ui/filter-box";
import { adminCardClass, adminInputClass } from "@/lib/design";
import { SettingsDropdown } from "../settings/settings-dropdown";

export type TeamFilterState = {
  search: string;
  permissionProfileId: string;
  status: "" | "ativo" | "inativo";
};

type Props = {
  filters: TeamFilterState;
  onChange: (patch: Partial<TeamFilterState>) => void;
  onClear?: () => void;
  layout?: "inline" | "stacked";
};

export function TeamFilters({
  filters,
  onChange,
  onClear,
  layout = "inline",
}: Props) {
  const { options: staffRoles } = useStaffRoleOptions();
  const roleOptions = [
    { value: "", label: "Função" },
    ...staffRoles.map((r) => ({ value: r.value, label: r.label })),
  ];
  const statusOptions = TeamServiceMock.getStatusOptions();
  const isStacked = layout === "stacked";
  const fieldClass = isStacked ? "w-full" : "min-w-0 flex-1 lg:max-w-[200px]";
  const statusFieldClass = isStacked ? "w-full" : "min-w-0 flex-1 lg:max-w-[180px]";
  const active = filtersActive([
    filters.search,
    filters.permissionProfileId,
    filters.status,
  ]);

  const fields = (
    <>
      <div className={isStacked ? "relative w-full" : "relative min-w-[200px] flex-[2]"}>
        <HiMagnifyingGlass
          className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-neutral-400"
          aria-hidden
        />
        <input
          type="search"
          placeholder="Buscar por nome, e-mail ou usuário…"
          value={filters.search}
          onChange={(e) => onChange({ search: e.target.value })}
          className={`${adminInputClass} ${filterFieldHeightClass} w-full pl-10`}
          aria-label="Busca"
        />
      </div>
      <div className={fieldClass}>
        <SettingsDropdown
          aria-label="Função"
          value={filters.permissionProfileId}
          options={roleOptions}
          onSelect={(permissionProfileId) =>
            onChange({ permissionProfileId })
          }
        />
      </div>
      <div className={statusFieldClass}>
        <SettingsDropdown
          aria-label="Status"
          value={filters.status}
          options={statusOptions}
          onSelect={(status) =>
            onChange({ status: status as TeamFilterState["status"] })
          }
        />
      </div>
    </>
  );

  if (isStacked) {
    return (
      <div className={`${adminCardClass} p-4`}>
        <div className="flex flex-col gap-4">{fields}</div>
      </div>
    );
  }

  return (
    <FilterBox active={active} onClear={onClear}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch">{fields}</div>
    </FilterBox>
  );
}
