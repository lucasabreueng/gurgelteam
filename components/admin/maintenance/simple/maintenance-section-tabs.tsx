...
import type { MaintenancePageTabKey } from "@/lib/contracts/maintenance/complete-checklist";
import { MaintenanceServiceMock } from "@/services/maintenance/maintenanceServiceMock";

type Props = {
  active: MaintenancePageTabKey;
  onChange: (key: MaintenancePageTabKey) => void;
};
...
export function MaintenanceSectionTabs({ active, onChange }: Props) {
  return (
    <nav aria-label="Seções de manutenção" className="admin-page-gutter select-none">
      <div role="tablist" className={TAB_LIST_CLASS}>
        {MaintenanceServiceMock.getMaintenancePageTabs().map((tab) => {
