...
const TYPE_OPTIONS: { value: MovementType | ""; label: string }[] = [
  ...
  ...(
    Object.entries(InventoryServiceMock.getMovementTypeLabels()) as [
      MovementType,
      string,
    ][]
...
type Props = {
  ...
  onChange: (patch: Partial<MovementsFilterState>) => void;
  onClear?: () => void;
...
export function MovementsFilters({
  ...
  const active = filtersActive([filters.query, filters.typeFilter]);
    ...
    <>
      ...
      <div className={fieldWrap}>
        <SettingsDropdown
          ...
          onSelect={(typeFilter) => onChange({ typeFilter: typeFilter as MovementType | "" })}
        />
      </div>
    </>
  );
...
export function countMovementsFilters(filters: MovementsFilterState): number {
  return countActiveFilters([filters.query, filters.typeFilter]);
}
