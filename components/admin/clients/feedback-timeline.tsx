...
function eventColor(type: ClientTimelineEvent["type"]) {
  switch (type) {
    ...
    case "evento":
      return adminTimelineAccentClass;
    default:
      return "bg-white text-accent ring-black/10";
  }
}
...
export function FeedbackTimeline({ events }: Props) {
  return (
    <section>
      <h3 className={adminSubsectionTitleClass}>Timeline</h3>
        ...
        {events.map((event, i) => {
          ...
          return (
            <li key={event.id} className="relative pb-8 last:pb-0">
              ...
              <p className={`mt-1 ${adminTextAccentClass}`}>{event.title}</p>
