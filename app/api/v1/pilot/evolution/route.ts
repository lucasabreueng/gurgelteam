...
import { buildPilotEvolution } from "@/lib/server/pilot/build-pilot-area";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await buildPilotEvolution(user.clientId);
    if (!data) {
      return jsonError({
    ...
    console.error("[pilot/evolution GET]", error);
